...
public class DegradeRule extends AbstractRule {
     ...
     * 注意上面说到了half-open状态 ，是指经过熔断时长后  再发一个请求，如果跟这个额请求的响应时间小于慢调用RT则结熔断。
     *
     * 异常比例 (ERROR_RATIO)：当单位统计时长（statIntervalMs）内请求数目大于设置的最小请求数目，并且异常的比例大于阈值，
     * 则接下来的熔断时长内请求会自动被熔断。经过熔断时长后熔断器会进入探测恢复状态（HALF-OPEN 状态），若接下来的一个请求成功完成（没有错误）
     * 则结束熔断，否则会再次被熔断。异常比率的阈值范围是 [0.0, 1.0]，代表 0% - 100%。
