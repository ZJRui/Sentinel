 ...
 * }
 * ContextUtil.exit();
 * 这里有个问题，如果我们直接调用Sphu enter而没有调用ContextUtil.enter,n那么在enter
 * 的时候会判断当前线程中有没有Context，如果没有则新创建，问题是这个新创建的context什么时候调用他的exit方法？
 *
 * </pre>
 * <p>
 ...
 * {@link ClusterNode}.
 * 不同的context同一个Resource会有多个DefaultNode，但是只有一个ClusterNode
 * </p>
 * <p>
...
*
记住，将共享相同的资源({@link ResourceWrapper#equals(Object)})
*全局相同的{@link processslotchain}，无论在哪个上下文中。因此,如果
*代码进入{@link #entry(Context, ResourceWrapper, DefaultNode, int, Object…)}，
资源名必须相同，但上下文名可以不同。
*
*如果我们使用{@link com.alibaba.csp.sentinel。SphU #入口(字符串资源)}
*在不同的上下文中输入相同的资源，使用上下文名作为映射键可以
*区分同一资源。在这种情况下，将创建多个{@link DefaultNode}
*相同的资源名，每个不同的上下文(不同的上下文名)。
*
考虑另一个问题。一个资源可以有多个{@link DefaultNode}，
*那么获得相同资源的全部统计数据的最快方法是什么?
*答案是所有具有相同资源名的{@link DefaultNode}共享一个
* {@link ClusterNode}。详情请参阅{@link ClusterBuilderSlot}。
